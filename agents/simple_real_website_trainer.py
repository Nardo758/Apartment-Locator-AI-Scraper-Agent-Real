#!/usr/bin/env python3
"""
Simple Real Website Trainer
Direct rental data extraction from real websites using Chrome profile authentication
"""
...
class SimpleRealWebsiteTrainer:
    """Simple trainer that directly extracts rental data from real websites"""
    ...
    async def extract_real_rental_data(self, url: str, profile_name: str = "Default"):
        """Extract rental data directly from real website"""
        print("[SIMPLE] Simple Real Website Rental Data Extractor")
        print("[SIMPLE] Using authenticated Chrome profile for direct data extraction")
        print("=" * 70)
        ...
        async with async_playwright() as p:
            try:
                ...
                print("[SIMPLE] Authenticated Chrome profile loaded")
                print(f"[SIMPLE] Navigating to: {url}")
                ...
                print("[SIMPLE] Waiting for dynamic content to load...")
                await asyncio.sleep(5)

                # Extract rental data directly
                rental_data = await self._extract_rental_data_directly(page)
                ...
                result = {
                    "extraction_session": {
                        ...
                        "extraction_method": "direct_authenticated_chrome",
                ...
                print("[DATA] Direct extraction complete:")
                print(f"   [TITLE] {title}")
    ...
    async def _extract_rental_data_directly(self, page):
        """Extract rental data using direct comprehensive strategies"""
        data = {
            "prices": [],
            "units": [],
            "features": [],
        ...
        try:
            ...
            for selector in price_selectors:
                try:
                    ...
                    for i in range(min(count, 10)):  # Limit to first 10 matches per selector
...
async def main():
    """Extract rental data from real website"""
    if len(sys.argv) < 2:
        print("Usage: python simple_real_website_trainer.py <url> [profile_name]")
        print("Example: python simple_real_website_trainer.py https://highlandsatsweetwatercreek.com/")
        print("Example: python simple_real_website_trainer.py https://highlandsatsweetwatercreek.com/ Profile\\ 1")
        print()
    ...
    print("[SIMPLE] Simple Real Website Rental Data Extractor")
    print("[SIMPLE] Direct extraction from live rental websites")
    print("=" * 50)

    trainer = SimpleRealWebsiteTrainer()

    try:
        result = await trainer.extract_real_rental_data(url, profile_name)

        if result:
            print("\n[SUCCESS] Real website data extraction completed!")

            # Save comprehensive results
            timestamp = int(time.time())
            filename = f"simple_real_extraction_{timestamp}.json"

            with open(filename, 'w', encoding='utf-8') as f:
                json.dump(result, f, indent=2, ensure_ascii=False)

            print(f"[SAVE] Extraction results saved: {filename}")

            # Show summary
            rental_data = result.get('rental_data', {})

            print("\n[SUMMARY] Real Website Extraction Results:")
            print(f"   Website: {result['extraction_session']['page_title']}")
            ...
            print("\n[ERROR] Real website extraction failed")

    except KeyboardInterrupt:
        print("\n[STOP] Real website extraction stopped")
    except Exception as e:
        print(f"\n[ERROR] Real website extraction error: {e}")
