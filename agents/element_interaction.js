...
class RobustElementInteractor {
  ...
  async waitForElement(selector, timeout = this.timeout) {
    ...
    while (Date.now() - start < timeout) {
      ...
      const doc = document.scrollingElement || document.documentElement ||
        document.body;
      const maxScroll = (doc.scrollHeight || document.body.scrollHeight) -
  ...
  async interact(
    ...
    const _scoring = this.validateSelectorSpecificity(
      elementSelectors.map((s) => `${containerSelector} ${s}`),
    );
    ...
    if (!element) {
      throw new Error(
        `Element not found with selectors: ${elementSelectors.join(", ")}`,
      );
    }
  ...
  async interactDetailed(
    ...
    const scoring = this.validateSelectorSpecificity(
      elementSelectors.map((s) => `${containerSelector} ${s}`),
    );
    ...
    if (!element) {
      return { ok: false, reason: "element-not-found", _scoring: scoring };
    }
    ...
    for (const s of elementSelectors) {
      try {
        ...
        if (
          candidate && candidate.isSameNode && candidate.isSameNode(element)
        ) {
          matchedSelector = s;
          break;
        }
    ...
    if (beforeHref && afterHref && beforeHref !== afterHref) {
    ...
    try {
      const modal = document.querySelector(
        '.modal.show, [role="dialog"].show, #fp-modal, .modal[aria-hidden="false"], .modal[aria-modal="true"]',
      );
      if (modal) {
        ...
        if (dlg) {
          actionType = "modal";
          modalHtml = dlg.outerHTML;
        }
      }
    } catch (_e) { /* ignore DOM read errors */ }
  ...
  _log(...args) {
    if (this.options && this.options.logging) {
      try {
        console.debug("[RobustInteractor]", ...args);
      } catch (_e) { /* ignore */ }
    }
  }
  ...
  async _findElement(container, selectors, recoveryStrategies = []) {
    this._log("Attempting findElement", {
      containerSelector: container &&
        (container.getAttribute ? container.getAttribute("class") : "unknown"),
      selectors,
    });
    const res = await this.findElement(
      container,
      selectors,
      recoveryStrategies,
    );
    this._log("findElement result", {
      found: !!res,
      tag: res ? res.tagName : null,
    });
    return res;
  }
  ...
  async _executeAction(element, action, value = null) {
    ...
    try {
      ...
      this._log("Action error", _e.message);
      throw _e;
    }
  }
}
