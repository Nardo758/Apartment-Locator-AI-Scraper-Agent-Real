...
const scriptContent = fs.readFileSync(
  path.resolve(__dirname, "..", "element_interaction.js"),
  "utf8",
);
...
describe("RobustElementInteractor (JSDOM)", () => {
  test("finds element with primary selector and clicks", async () => {
    ...
    const { RobustElementInteractor: _RobustElementInteractor } = window;
    const interactor = new window.RobustElementInteractor({ timeout: 1000 });
    const result = await interactor.interact("form#user-settings", [
  ...
  test("validates container existence", async () => {
    ...
    await expect(interactor.interact("form#non-existent", [".btn"], "click"))
  ...
  test("handles dynamic content simulation", async () => {
    const html = `
        ...
        `;
    const window = createWindow(html);
    const interactor = new window.RobustElementInteractor({
      timeout: 1000,
      retryInterval: 50,
    });
