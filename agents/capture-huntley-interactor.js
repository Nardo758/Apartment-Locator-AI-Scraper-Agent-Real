...
async function runCapture() {
  ...
  try {
    ...
    await page.addScriptTag({
      path: path.resolve(__dirname, "element_interaction.js"),
    });
    ...
    const result = await page.evaluate(async (container) => {
      if (!globalThis.RobustElementInteractor) {
        return { ok: false, reason: "interactor-missing" };
      }
      ...
      if (typeof interactor.interactDetailed === "function") {
        return await interactor.interactDetailed(
          `#${container}`,
          selectors,
          "click",
        );
      ...
      // fallback
      const res = await interactor.interact(
        `#${container}`,
        selectors,
        "click",
      );
      const matched = selectors.find((s) =>
        document.querySelector(`#${container} ${s}`)
      );
      const el = matched
        ? document.querySelector(`#${container} ${matched}`)
        : null;
    ...
    const videoFile = videos.length
      ? path.join(outDir, videos.sort().pop())
      : null;
