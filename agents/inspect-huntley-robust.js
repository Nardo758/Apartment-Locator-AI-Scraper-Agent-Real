...
(async () => {
  ...
  try {
    console.log(
      "🔎 Going to https://www.thehuntley.com/floorplans with domcontentloaded",
    );
    ...
    for (let i = 0; i < 8; i++) {
      await page.evaluate(() =>
        globalThis.scrollBy(0, globalThis.innerHeight / 2)
      );
    ...
    const candidates = await page.evaluate(() => {
      ...
      return nodes.map((el) => {
        ...
        return {
          ...
          sampleButtons: btns.slice(0, 5).map((b) => ({
      ...
      }).filter((x) => x.btnCount > 0).sort((a, b) => b.btnCount - a.btnCount)
        .slice(0, 30);
    });
