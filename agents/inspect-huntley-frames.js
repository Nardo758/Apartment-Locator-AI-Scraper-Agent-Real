...
(async () => {
  ...
  try {
    console.log(
      "🔎 Navigating to https://www.thehuntley.com/floorplans (non-headless)",
    );
    ...
    for (const frame of frames) {
      try {
        ...
        const candidates = await frame.evaluate(() => {
          ...
          }).filter((c) => c.btnCount > 0).sort((a, b) =>
            b.btnCount - a.btnCount
          ).slice(0, 20);
          return results;
        });
        ...
        frameInfos.push({
          name: frame.name(),
          url: frame.url(),
          error: _e.message, // normalize catch variable
        });
      }
    }
    ...
    console.error("Error during frame inspection:", _e); // normalize catch variable
  } finally {
