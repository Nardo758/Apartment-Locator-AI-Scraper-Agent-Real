...
async function testHuntleyInteractor() {
  ...
  try {
    ...
    await page.goto("https://www.thehuntley.com/floorplans", {
    ...
    try {
      await page.waitForSelector('div[id^="fp-container-"]', {
        timeout: 20000,
      });
    } catch (_e) {
      console.warn(
        "No fp-container detected within timeout; proceeding to scroll and search",
      );
    }

    for (let i = 0; i < 6; i++) {
      await page.evaluate(() =>
        globalThis.scrollBy(0, globalThis.innerHeight * 0.6)
      );
    ...
    try {
      clickResult = await page.evaluate(async (containerId) => {
        if (!globalThis.RobustElementInteractor) {
          throw new Error("RobustElementInteractor not available");
        }
        const interactor = new globalThis.RobustElementInteractor({
          timeout: 15000,
          logging: true,
        });
        return await interactor.interact(`#${containerId}`, [
          "a.track-apply",
    ...
    const pageStructure = await page.evaluate(() => {
      const containers = document.querySelectorAll(
        'div[id^="fp-container-"], div.fp-container',
      );
      return Array.from(containers).slice(0, 8).map((c) => ({
        id: c.id,
        classes: c.className,
        buttons: Array.from(c.querySelectorAll("a,button")).map((b) => ({
          text: (b.textContent || "").trim().replace(/\s+/g, " "),
          classes: b.className,
          attrs: Array.from(b.attributes || []).map((a) => ({
            name: a.name,
            value: a.value,
          })),
        })),
