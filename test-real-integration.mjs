...
async function testRealIntegration() {
  ...
  try {
    ...
    for (const [index, property] of scrapedProperties.entries()) {
      try {
        console.log(
          `\n🏘️  Processing property ${
            index + 1
          }/${scrapedProperties.length}: ${
            property.name || property.external_id
          }`,
        );
        ...
        console.error(
          `❌ Failed to transform property ${property.external_id}:`,
          error.message,
        );
        results.failed++;
        results.errors.push({
...
function displayTransformationResults(original, transformed) {
  console.log(
    `  📊 Original: $${original.current_price} | AI: $${transformed.ai_price} | Effective: $${transformed.effective_price}`,
  );
  console.log(
    `  🏠 ${transformed.bedrooms}BR/${transformed.bathrooms}BA | ${
      transformed.sqft || "N/A"
    } sqft`,
  );
  console.log(
    `  🎯 Amenities: ${transformed.amenities.length} | Features: ${transformed.features.length}`,
  );
  console.log(
    `  📈 Market Position: ${transformed.apartment_iq_data.market_position} (${transformed.apartment_iq_data.competitiveness_score}/100)`,
  );
  console.log(
    `  🌍 Location: ${transformed.city}, ${transformed.state} (${transformed.latitude}, ${transformed.longitude})`,
  );
}
...
async function testDatabaseOperations(transformedProperties) {
  ...
  for (const property of transformedProperties.slice(0, 3)) {
    ...
    console.log(
      ...
      } ${property.external_id}: ${validationResults.issues.length} issues`,
    );
  ...
  if (TEST_CONFIG.ENABLE_WRITE_BACK) {
    ...
    console.log(
      "  ℹ️  Write-back disabled (set ENABLE_WRITE_BACK=true to enable)",
    );
  }
}
...
async function testGeographicSearchWithRealData(transformedProperties) {
  ...
  const geoProperties = transformedProperties.filter((p) =>
  ...
  if (geoProperties.length > 0) {
    ...
    console.log(
      `    📍 Center: ${centerProperty.latitude}, ${centerProperty.longitude}`,
    );
    ...
    console.log(
      `  📊 Found ${searchResults.length} properties within 10 miles`,
    );
...
function analyzePerformance(results, transformedProperties) {
  const avgOriginalPrice =
    transformedProperties.reduce((sum, p) => sum + p.original_price, 0) /
    transformedProperties.length;
  const avgAiPrice =
    transformedProperties.reduce((sum, p) => sum + (p.ai_price || 0), 0) /
    transformedProperties.length;
  const avgEffectivePrice =
    transformedProperties.reduce((sum, p) => sum + p.effective_price, 0) /
    transformedProperties.length;

  console.log(`  💰 Average Original Price: $${Math.round(avgOriginalPrice)}`);
  console.log(`  🤖 Average AI Price: $${Math.round(avgAiPrice)}`);
  console.log(
    `  💡 Average Effective Price: $${Math.round(avgEffectivePrice)}`,
  );
  console.log(
    `  📊 AI Price Premium: ${
      (((avgAiPrice - avgOriginalPrice) / avgOriginalPrice) * 100).toFixed(1)
    }%`,
  );
  console.log(
    `  🎯 Properties with Market Intelligence: ${
      transformedProperties.filter((p) => p.apartment_iq_data).length
    }/${transformedProperties.length}`,
  );
  console.log(
    `  🌍 Properties with Coordinates: ${
      transformedProperties.filter((p) => p.latitude && p.longitude).length
    }/${transformedProperties.length}`,
  );
}
...
function displayFinalResults(results) {
  ...
  console.log(
    `📈 Success Rate: ${
      ((results.successful / results.tested) * 100).toFixed(1)
    }%`,
  );
