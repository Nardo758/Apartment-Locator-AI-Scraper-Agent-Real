...
class FrontendIntegrationTester {
  ...
  private async testDataTransformation(): Promise<void> {
    ...
    try {
      ...
      if (propError) {
        this.addResult(
          "Transformation: Properties data",
          false,
          `Error fetching properties: ${propError.message}`,
        );
      } else {
        this.addResult(
          "Transformation: Properties data",
        ...
        if (properties && properties.length > 0) {
          ...
          const missingFields = requiredFields.filter((field) =>
            firstProperty[field] === null || firstProperty[field] === undefined
          );

          if (missingFields.length === 0) {
            this.addResult(
              "Transformation: Required fields",
              true,
    ...
    try {
      ...
      if (error) {
        ...
        this.addResult(
          "Geographic: Search function",
          true,
          `Found ${data?.length || 0} properties within radius`,
        );
        ...
        if (data && data.length > 0) {
          ...
          if (result.distance_km !== null && result.distance_km <= 25) {
            this.addResult(
              ...
              true,
              `Distance within expected range: ${result.distance_km}km`,
            );
          } else {
            this.addResult(
              "Geographic: Distance calculation",
              false,
              `Distance calculation issue: ${result.distance_km}km`,
            );
          }
        }
      }
      ...
      this.addResult(
        "Geographic: Overall",
        false,
        `Geographic search test failed: ${error.message}`,
      );
    }
  }
  ...
  private async testMatchScoring(): Promise<void> {
    ...
    try {
      ...
      if (profileError) {
        this.addResult(
          "Match Scoring: Test user creation",
          false,
      ...
      const { data: matchScore, error: matchError } = await this.supabase.rpc(
        "calculate_property_match_score",
        {
      ...
      if (matchError) {
        this.addResult(
          "Match Scoring: Score calculation",
          false,
          `Match score error: ${matchError.message}`,
        );
      } else {
      ...
      this.addResult(
        "Match Scoring: Overall",
        false,
        `Match scoring test failed: ${error.message}`,
      );
    }
  }
    ...
    try {
      ...
      if (error) {
        ...
        this.addResult(
          "Market Intelligence: Data access",
          true,
          `Found ${data?.length || 0} market intelligence records`,
        );
        ...
        if (data && data.length > 0) {
          ...
          if (hasRequiredFields) {
            this.addResult(
              ...
              true,
              "Market intelligence has required fields",
            );
          } else {
            this.addResult(
              "Market Intelligence: Data structure",
              false,
              "Missing required fields in market intelligence",
            );
          }

          // Validate data freshness
          const calculatedAt = new Date(record.calculated_at);
          const daysSinceCalculated = (Date.now() - calculatedAt.getTime()) /
            (1000 * 60 * 60 * 24);

          if (daysSinceCalculated <= 30) {
            this.addResult(
              "Market Intelligence: Data freshness",
              true,
              `Data is ${Math.round(daysSinceCalculated)} days old`,
            );
          } else {
            this.addResult(
              "Market Intelligence: Data freshness",
              false,
              `Data is stale: ${Math.round(daysSinceCalculated)} days old`,
            );
          }
        }
      }
      ...
      this.addResult(
        "Market Intelligence: Overall",
        false,
        `Market intelligence test failed: ${error.message}`,
      );
    }
  }
  ...
  private async testUserProfiles(): Promise<void> {
    ...
    try {
      ...
      if (insertError) {
        this.addResult(
          "User Profiles: Profile creation",
          false,
        ...
        const { data: retrievedProfile, error: selectError } = await this
          .supabase
          .from("user_profiles")
          .select("*")
          .eq("user_id", testUserId)
        ...
        if (selectError) {
          this.addResult(
            "User Profiles: Profile retrieval",
            false,
          ...
          if (
            retrievedProfile.ai_preferences && retrievedProfile.search_criteria
          ) {
            this.addResult(
              "User Profiles: JSONB fields",
              true,
              "JSONB fields stored and retrieved correctly",
            );
          } else {
            this.addResult(
              "User Profiles: JSONB fields",
              false,
              "JSONB fields not stored correctly",
            );
          }
        }
      ...
      this.addResult(
        "User Profiles: Overall",
        false,
        `User profiles test failed: ${error.message}`,
      );
    }
  }
  ...
  private async testRentalOffers(): Promise<void> {
    ...
    try {
      ...
      if (insertError) {
        ...
        if (selectError) {
          ...
          this.addResult(
            "Rental Offers: Offer retrieval",
            true,
            `Retrieved ${offers?.length || 0} offers`,
          );
        }
      ...
      this.addResult(
        "Rental Offers: Overall",
        false,
        `Rental offers test failed: ${error.message}`,
      );
    }
  }
  ...
  private async testDataIntegrity(): Promise<void> {
    ...
    try {
      ...
      if (joinError) {
        this.addResult(
          "Data Integrity: Foreign key joins",
          false,
