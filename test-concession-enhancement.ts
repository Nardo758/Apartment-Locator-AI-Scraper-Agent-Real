...
import {
  ...
  ConcessionTracker,
  parseConcessionValue,
} from "./src/services/concession-tracker.ts";
...
const detectedConcessions = ConcessionDetector.detectConcessionKeywords(
  testHtml,
);
...
const effectiveRent = ConcessionDetector.calculateNetEffectiveRent(
  baseRent,
  freeRentOffer,
);
...
const marketStats = await ConcessionTracker.trackMarketConcessions(
  mockProperties,
);
...
console.log("\n📋 Test 8: Concession Confidence Calculation");
const confidence = ConcessionDetector.calculateConcessionConfidence(
  mockIntelligence,
);
console.log(
  `Concession confidence score: ${confidence.toFixed(2)} (${
...
console.log("\n🎯 Summary:");
console.log(
  `• Keyword detection: ${detectedConcessions.length} concessions found`,
);
console.log(
  `• Context extraction: ${Object.keys(context).length} context sections`,
);
console.log(
  `• Effective rent calculation: $${baseRent} → $${effectiveRent} (${
    ((baseRent - effectiveRent) / baseRent * 100).toFixed(1)
  }% discount)`,
);
console.log(
  `• Market concession rate: ${marketStats.concession_rate.toFixed(1)}%`,
);
