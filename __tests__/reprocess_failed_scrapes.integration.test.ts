...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  console.warn(
    "Skipping reprocess_failed_scrapes integration test: SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY not set",
  );
  test("skip", () => {
  ...
  describe("reprocess_failed_scrapes RPC integration", () => {
    ...
    beforeAll(async () => {
      try {
        ...
        await supabase.from("failed_scrapes").delete().eq(
          "external_id",
          testExternalId,
        );
        ...
        const { data, error } = await supabase.from("failed_scrapes").insert({
        ...
        if (error) {
          ...
          if (
            ...
            console.warn(
              "Skipping reprocess integration: failed_scrapes table not present",
            );
    ...
    afterAll(async () => {
      ...
      await supabase.from("scraping_queue").delete().eq(
        "external_id",
        testExternalId,
      );
      await supabase.from("failed_scrapes").delete().eq(
    ...
    test(
      ...
      async () => {
        if (skipIntegration) {
          console.warn(
            "Skipped reprocess integration test (no failed_scrapes table)",
          );
        ...
        const { data: queueRow, error: queueErr } = await supabase.from(
          "scraping_queue",
        ).select("*").eq("external_id", testExternalId).limit(1).single();
        if (
          queueErr && queueErr.code !== "PGRST116"
        ) { /* allow no rows if skipped */ }

        // Verify failed_scrapes requeue_count incremented (if it was reprocessed)
        const { data: failedRow } = await supabase.from("failed_scrapes")
          .select("requeue_count").eq("id", insertedId).limit(1).single();
