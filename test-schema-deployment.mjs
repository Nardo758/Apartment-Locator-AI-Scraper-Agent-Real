...
async function calculateAiPrice(scrapedData) {
  try {
    ...
    const luxuryCount = amenities.filter((a) =>
      luxuryAmenities.some((luxury) =>
        a.toLowerCase().includes(luxury)
      )
    ).length;
...
async function calculateEffectivePrice(scrapedData) {
  try {
    ...
    if (scrapedData.free_rent_concessions) {
      const concessionValue = parseConcessionValue(
  ...
  for (const text of textFields) {
    if (typeof text === "string") {
      ...
      for (const amenity of commonAmenities) {
        if (
          lowerText.includes(amenity.toLowerCase()) &&
...
async function extractPetPolicy(scrapedData) {
  ...
  for (const text of textFields) {
    if (typeof text === "string") {
      ...
      if (
        lowerText.includes("pet friendly") || lowerText.includes("pets allowed")
      ) {
        return "Pets Allowed";
      }
...
async function generateIqData(scrapedData) {
  try {
    ...
    const competitivenessScore = Math.round(
      (confidenceScore * 0.4 +
        (demandLevel === "high" ? 0.9 : demandLevel === "medium" ? 0.6 : 0.3) *
          0.3 +
        (marketPosition === "below_market"
            ? 0.9
...
async function testDataTransformation(results) {
  ...
  try {
    ...
    const validations = [
      {
        test: "External ID preserved",
        condition: frontendProperty.external_id === testProperty.external_id,
      },
      {
        test: "Name transformed",
        condition: frontendProperty.name === testProperty.name,
      },
      {
        test: "Address preserved",
        condition: frontendProperty.address === testProperty.address,
      },
      {
        test: "Geographic data preserved",
        condition: frontendProperty.latitude === testProperty.latitude,
      },
      {
        test: "Pricing data transformed",
        condition:
          frontendProperty.original_price === testProperty.current_price,
      },
      {
        test: "AI price calculated",
        condition: frontendProperty.ai_price !== undefined &&
          frontendProperty.ai_price > 0,
      },
      {
        test: "Effective price calculated",
        condition: frontendProperty.effective_price !== undefined,
      },
      {
        test: "Amenities extracted",
        condition: Array.isArray(frontendProperty.amenities) &&
          frontendProperty.amenities.length > 0,
      },
      {
        test: "Features extracted",
        condition: Array.isArray(frontendProperty.features) &&
          frontendProperty.features.length > 0,
      },
      {
        test: "Pet policy determined",
        condition: frontendProperty.pet_policy !== undefined,
      },
      {
        test: "Apartment IQ generated",
        condition: frontendProperty.apartment_iq_data !== undefined,
      },
    ];
    ...
    if (TEST_CONFIG.VERBOSE_OUTPUT) {
      ...
      console.log(
        `    Amenities: ${frontendProperty.amenities?.length || 0} items`,
      );
      console.log(
        `    Features: ${frontendProperty.features?.length || 0} items`,
      );
      console.log(`    Pet Policy: ${frontendProperty.pet_policy}`);
      console.log(`    Parking: ${frontendProperty.parking}`);
      console.log(
        `    Market Position: ${
          frontendProperty.apartment_iq_data?.market_position || "N/A"
        }`,
      );
      console.log(
        `    Competitiveness Score: ${
          frontendProperty.apartment_iq_data?.competitiveness_score || "N/A"
        }`,
      );
    }
  } catch (error) {
...
async function testAiPriceCalculation(results) {
  ...
  try {
    ...
    for (const testCase of testCases) {
      ...
      const inRange = aiPrice >= testCase.expectedRange[0] &&
        aiPrice <= testCase.expectedRange[1];

      if (inRange) {
        console.log(
          `  ✅ ${testCase.name}: $${aiPrice} (expected $${
  ...
  try {
    ...
    for (const testCase of testCases) {
      ...
      const withinTolerance =
        Math.abs(effectivePrice - testCase.expected) <= tolerance;

      if (withinTolerance) {
        console.log(
          `  ✅ ${testCase.name}: $${effectivePrice} (expected ~$${
            Math.round(testCase.expected)
          })`,
        );
        results.passed += 1;
      } else {
  ...
  try {
    ...
    for (const scenario of testScenarios) {
      ...
      const validations = [
        {
          ...
          condition: iqData.market_position === scenario.expectedPosition,
        },
        {
          test: "Demand level",
          condition: iqData.demand_level === scenario.expectedDemand,
        },
        {
          test: "Confidence score valid",
          condition: iqData.confidence_score >= 0 &&
            iqData.confidence_score <= 1,
        },
        {
          test: "Competitiveness score valid",
          condition: iqData.competitiveness_score >= 0 &&
            iqData.competitiveness_score <= 100,
        },
        {
          test: "Recommendation provided",
          condition: iqData.recommendation && iqData.recommendation.length > 0,
        },
      ];
      ...
      if (TEST_CONFIG.VERBOSE_OUTPUT) {
        ...
        console.log(
          `    📊 Confidence: ${(iqData.confidence_score * 100).toFixed(1)}%`,
        );
        console.log(
          `    🏆 Competitiveness: ${iqData.competitiveness_score}/100`,
        );
...
async function testBatchProcessing(results) {
  ...
  try {
    ...
    const batchData = Array.from(
      { length: TEST_CONFIG.SAMPLE_SIZE },
      (_, i) => ({
    ...
    const validations = [
      {
        test: "Batch size preserved",
        condition: transformedBatch.length === batchData.length,
      },
      {
        test: "All properties transformed",
        condition: transformedBatch.every((p) =>
          p.external_id && p.original_price
        ),
      },
      {
        test: "AI prices calculated",
        condition: transformedBatch.filter((p) => p.ai_price).length > 0,
      },
      {
        test: "Effective prices calculated",
        condition: transformedBatch.every((p) =>
      ...
      {
        test: "No duplicate external IDs",
        condition: new Set(transformedBatch.map((p) =>
          p.external_id
        )).size === transformedBatch.length,
      },
    ];
    ...
    console.log(
      `  📊 Processed ${transformedBatch.length} properties in ${processingTime}ms`,
    );
    console.log(
      `  ⚡ Average processing time: ${
        (processingTime / transformedBatch.length).toFixed(2)
      }ms per property`,
    );
  } catch (error) {
    ...
    for (const testCase of qualityTestCases) {
      try {
        ...
        const qualityChecks = [
          ...
          frontendProperty.external_id.length > 0,
        ];

        qualityScore = qualityChecks.filter(Boolean).length /
          qualityChecks.length;

        const qualityLevel = qualityScore >= 0.8
          ? "high"
          : qualityScore >= 0.6
          ? "medium"
          : "low";
        ...
        if (qualityMet) {
          console.log(
            `  ✅ ${testCase.name}: ${qualityLevel} quality (${
          ...
          console.log(
            `  ❌ ${testCase.name}: ${qualityLevel} quality, expected ${testCase.expectedQuality}`,
          );
          results.failed += 1;
        }
        results.total += 1;
        ...
        console.log(
          `  ❌ ${testCase.name}: Transformation failed - ${error.message}`,
        );
        results.failed += 1;
        results.total += 1;
      }
...
async function testGeographicData(results) {
  ...
  try {
    ...
    for (const testCase of geoTestCases) {
      ...
      const validations = [
        {
          test: "Latitude preserved",
          condition: frontendProperty.latitude === testCase.lat,
        },
        {
          test: "Longitude preserved",
          condition: frontendProperty.longitude === testCase.lng,
        },
        {
          test: "City preserved",
          condition: frontendProperty.city === testCase.city,
        },
        {
          test: "State preserved",
          condition: frontendProperty.state === testCase.state,
        },
        {
          test: "ZIP preserved",
          condition: frontendProperty.zip === testCase.zip,
        },
      ];
