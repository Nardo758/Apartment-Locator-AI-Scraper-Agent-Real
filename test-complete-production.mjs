...
async function testSchemaDeployment() {
  ...
  try {
    ...
    if (fs.existsSync("manual-deployment.sql")) {
      ...
      console.log(
        `  📊 Schema completeness: ${tablesFound}/${requiredTables.length} tables, ${functionsFound}/${requiredFunctions.length} functions`,
      );
    }
    ...
    console.log(
      `  🎯 Schema Test: ${TEST_RESULTS.schema.status.toUpperCase()}\n`,
    );
  } catch (error) {
...
async function testFunctionUpdates() {
  ...
  try {
    ...
    if (fs.existsSync("supabase/functions/ai-scraper-worker/index.ts")) {
      ...
      console.log(
        `  📊 Integration completeness: ${featuresFound}/${integrationFeatures.length} features`,
      );
    }

    TEST_RESULTS.functions.status = functionsReady ? "passed" : "warning";
    TEST_RESULTS.functions.details = [
      `Functions updated with frontend integration`,
    ];

    console.log(
      `  🎯 Functions Test: ${TEST_RESULTS.functions.status.toUpperCase()}\n`,
    );
  } catch (error) {
...
async function testEndToEndIntegration() {
  ...
  try {
    ...
    console.log(
      `    Market Position: ${transformedData.apartment_iq_data.market_position}`,
    );
    console.log(
      `    Competitiveness: ${transformedData.apartment_iq_data.competitiveness_score}/100`,
    );
    ...
    TEST_RESULTS.integration.status = validationResults.valid
      ? "passed"
      : "failed";
    TEST_RESULTS.integration.details = validationResults.issues;

    console.log(
      `  🎯 Integration Test: ${TEST_RESULTS.integration.status.toUpperCase()}\n`,
    );
  } catch (error) {
...
async function simulateTransformation(scrapedData) {
  ...
  if (scrapedData.amenities.some((a) => ["pool", "gym"].includes(a))) {
    aiPrice *= 1.03;
  }
  ...
  if (
    ...
    scrapedData.free_rent_concessions.includes("1 month")
  ) {
    effectivePrice -= 200; // Rough concession value
  }
  if (scrapedData.application_fee) {
    effectivePrice += scrapedData.application_fee / 12;
  }

  // Simulate market intelligence
  const apartmentIqData = {
    market_position: scrapedData.current_price > 3000
      ? "above_market"
      : scrapedData.current_price < 2000
      ? "below_market"
      : "at_market",
...
function validateTransformation(transformedData) {
  ...
  if (
    transformedData.ai_price < transformedData.original_price * 0.8 ||
  ...
  if (
    !transformedData.apartment_iq_data.market_position ||
    !["below_market", "at_market", "above_market"].includes(
      transformedData.apartment_iq_data.market_position,
    )
  ) {
...
async function testPerformanceMetrics() {
  ...
  try {
    ...
    console.log(
      `    Average Time: ${avgTimePerProperty.toFixed(2)}ms per property`,
    );
    console.log(
      `    Throughput: ${
        (1000 / avgTimePerProperty).toFixed(1)
      } properties/second`,
    );
    ...
    const performancePassed =
      avgTimePerProperty < performanceTargets.avgTimePerProperty &&
    ...
    if (performancePassed) {
      ...
      console.log(
        `  ⚠️  Performance below targets (${performanceTargets.avgTimePerProperty}ms per property)`,
      );
    }

    TEST_RESULTS.performance.status = performancePassed ? "passed" : "warning";
    TEST_RESULTS.performance.details = [
      `${avgTimePerProperty.toFixed(2)}ms per property average`,
    ];

    console.log(
      `  🎯 Performance Test: ${TEST_RESULTS.performance.status.toUpperCase()}\n`,
    );
  } catch (error) {
...
function displayFinalSummary() {
  ...
  if (passedCount === totalCount) {
    ...
    console.log(
      ...
      } test(s) need attention before production deployment.`,
    );
