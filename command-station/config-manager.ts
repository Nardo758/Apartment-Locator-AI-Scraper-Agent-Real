...
export class ConfigManager {
  ...
  private static async loadFromSupabase(): Promise<
    Partial<SystemConfig> | null
  > {
    try {
      ...
      const response = await fetch(
        `${supabaseUrl}/rest/v1/system_config?key=eq.${this.CONFIG_KEY}`,
        {
          headers: {
  ...
  private static async saveToLocal(_config: SystemConfig): Promise<void> {
    try {
      await Promise.resolve();
      // In a real implementation, you might want to save to a local file
      // For now, we'll just log that local save was attempted
      console.log(
        "Local config save attempted (not implemented in this example)",
      );
    } catch (error) {
  ...
  private static validateConfig(config: unknown): void {
    ...
    if (
      typeof config.dailyCostLimit !== "number" || config.dailyCostLimit <= 0
    ) {
    ...
    if (config.alertThresholds) {
      ...
      if (
        typeof thresholds.errorRate !== "number" || thresholds.errorRate < 0 ||
      ...
      if (
        typeof thresholds.responseTime !== "number" ||
      ...
      if (
        typeof thresholds.costLimit !== "number" || thresholds.costLimit <= 0
      ) {
        errors.push("alertThresholds.costLimit must be a positive number");
      }

      if (
        typeof thresholds.memoryUsage !== "number" ||
        thresholds.memoryUsage < 0 || thresholds.memoryUsage > 1
      ) {
      ...
      integrations: {
        ...
        claude: {
          ...
          maxTokens: config.maxTokens ??
            currentConfig.integrations.claude.maxTokens,
        },
      },
    });
  }
  ...
  static async updateAlertThresholds(
    thresholds: Partial<SystemConfig["alertThresholds"]>,
  ): Promise<void> {
    const currentConfig = await this.getConfig();
    await this.saveConfig({
      ...currentConfig,
