      ...
      return new Response(
        JSON.stringify({
          ...
          costs: {
            ...
            remaining: Math.max(
              0,
              config.dailyCostLimit - costMetrics.daily_cost,
            ),
          },
          claude: {
  ...
  private static async getQueueStatus(): Promise<any> {
    try {
      ...
      return {
        ...
        last_completed: new Date(Date.now() - Math.random() * 3600000)
          .toISOString(),
    ...
    try {
      ...
      for (let i = 0; i < errorCount; i++) {
        errors.push({
          ...
          component:
            ["scraper", "claude", "database"][Math.floor(Math.random() * 3)],
        });
      }
  ...
  private static async checkDatabase(): Promise<boolean> {
    try {
      ...
      const response = await fetch(
        `${supabaseUrl}/rest/v1/properties?limit=1`,
        {
