  ...
  private static async getActiveAlerts(): Promise<any[]> {
    try {
      ...
      const alertTypes = [
        ...
        {
          level: "error",
          message: "Claude API rate limit approaching",
          component: "claude",
        },
        {
          level: "info",
          message: "Large batch processing in progress",
          component: "scraper",
        },
      ];
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          ...
          dataPoints,
          summary: {
            total_properties: dataPoints.reduce(
              (sum, point) => sum + point.properties_processed,
              0,
            ),
            avg_success_rate: dataPoints.reduce((sum, point) =>
              sum + point.success_rate, 0) / dataPoints.length,
            total_cost: dataPoints.reduce((sum, point) =>
              sum + point.cost, 0),
          },
        }),
        {
