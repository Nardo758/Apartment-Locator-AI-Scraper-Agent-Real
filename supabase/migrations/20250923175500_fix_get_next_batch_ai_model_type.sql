...
-- Drop the old function first to avoid Postgres error when changing RETURN TABLE column types
DROP FUNCTION IF EXISTS public.get_next_scraping_batch(integer);

CREATE FUNCTION public.get_next_scraping_batch(batch_size INTEGER DEFAULT 50)
RETURNS TABLE(
    queue_id BIGINT,
    external_id VARCHAR,
    url VARCHAR,
