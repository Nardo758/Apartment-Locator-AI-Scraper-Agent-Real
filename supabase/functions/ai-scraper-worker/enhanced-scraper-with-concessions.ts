...
async function enhancedScrapeWithConcessions(job: any) {
  try {
    ...
    console.log(
      `🔍 Quick concession scan: ${quickConcessions.length} offers found`,
    );
    ...
    const concessionConfidence = ConcessionDetector
      .calculateConcessionConfidence(propertyIntel);
...
async function scrapeCurrentListings(
  url: string,
  htmlContent: string,
): Promise<any[]> {
  ...
  const systemPrompt =
    `Extract ALL apartment listings from this property website HTML. 
  ...
  try {
    ...
    if (SUPABASE_URL && SUPABASE_KEY) {
      ...
      for (const apartment of scrapingResult.apartments) {
        const apartmentData = {
          external_id: `concession-${Date.now()}-${
      ...
      try {
        const concessionStats = await ConcessionTracker.trackMarketConcessions(
          scrapingResult.apartments,
        );
        await ConcessionTracker.saveConcessionAnalytics(
          supabase,
          concessionStats,
          "atlanta",
        );
      ...
      JSON.stringify({
        ...
        data: {
          ...
          concessions_detected:
            scrapingResult.scraping_metadata.concession_detected,
...
// Export the handler
export { enhancedScrapeWithConcessions, handleScrapingRequest };
