...
function simulateClaudeExtraction(property) {
  ...
  try {
    ...
    if (adminFeeMatch) {
      ...
      property.html.includes("admin fee waived") ||
      property.html.includes("admin fee waived")
    ) {
    ...
    for (const pattern of concessionPatterns) {
      const match = property.html.match(pattern);
      if (match) {
        const context = property.html.substring(
...
async function runTest() {
  ...
  for (let i = 0; i < testProperties.length; i++) {
    ...
    if (result.success) {
      ...
      console.log(
        `   🛏️  Beds/Baths: ${result.data.bedrooms}/${result.data.bathrooms}`,
      );
      ...
      if (result.data.free_rent_concessions) {
        console.log(
          `   🎁 Concessions: ${
            result.data.free_rent_concessions.substring(0, 50)
          }...`,
        );
      }
    } else {
  ...
  if (successCount === testProperties.length) {
    ...
    console.log(
      `   • Expected Total Time: ${Math.round(scaledTime / 60)} minutes`,
    );
    console.log(
