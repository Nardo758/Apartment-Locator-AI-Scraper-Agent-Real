...
function mockAiScraperResponse(property: TestProperty): MockResponse {
  ...
  const bedroomMatch = html.match(/(\d+)\s*(bed|bedroom)/i) ||
    html.match(/Studio/i);
  ...
  const bedrooms = bedroomMatch
    ? (bedroomMatch[0].includes("Studio") ? 0 : parseInt(bedroomMatch[1]))
    : 1;
...
async function runMockTests(): Promise<void> {
  ...
  console.log(
    "   For real testing, set up environment variables and run test-ai-scraper.ts",
  );
  console.log();
  ...
  for (let i = 0; i < testProperties.length; i += batchSize) {
    ...
    console.log(
      `📦 Processing batch ${Math.floor(i / batchSize) + 1}/${
        Math.ceil(testProperties.length / batchSize)
      }`,
    );
    ...
    for (const property of batch) {
      ...
      if (result.status === "ok") {
        ...
        if (result.data) {
          ...
          console.log(
            `     📍 ${data.name} - ${data.city}, ${data.state} - $${data.current_price}`,
          );
        }
        if (result.usage) {
          const usage = result.usage;
          console.log(
            `     💰 Tokens: ${usage.total_tokens}, Cost: $${usage.estimated_cost}`,
          );
        ...
        console.log(
          `  ❌ Property ${property.id} (${property.source}): ${result.message}`,
        );
      }
    }
  ...
  console.log(
    `   • Average cost per property: $${
      (results.totalCost / results.total).toFixed(4)
    }`,
  );
  console.log(
    `   • Average tokens per property: ${
      Math.round(results.totalTokens / results.total)
    }`,
  );
  ...
  console.log(
    "   2. Start: supabase functions serve ai-scraper-worker --env-file .env.local",
  );
  console.log(
    "   3. Run: deno run --allow-net --allow-env --allow-read test-ai-scraper.ts",
  );
}
