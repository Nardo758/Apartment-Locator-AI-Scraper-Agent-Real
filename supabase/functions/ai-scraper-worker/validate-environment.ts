...
class EnvironmentValidator {
    ...
    for (const envVar of requiredVars) {
      ...
      if (!value) {
        ...
        this.addResult(
          ...
          envVar.name,
          "fail",
          `Environment variable contains placeholder text`,
          "Please set actual value",
        );
      } else {
        ...
        if (envVar.name === "OPENAI_API_KEY" && !value.startsWith("sk-")) {
          ...
          envVar.name === "SUPABASE_URL" && !value.startsWith("https://")
        ) {
          this.addResult(
            "Environment",
            envVar.name,
    ...
    try {
      ...
      if (this.compareVersions(denoVersion, minVersion) >= 0) {
        ...
        this.addResult(
          ...
          "Deno",
          "fail",
          `Deno version ${denoVersion} is too old`,
          `Minimum required: ${minVersion}`,
        );
      }
    } catch (error) {
  ...
  async validateLocalFunction(): Promise<void> {
    ...
    try {
      ...
      this.addResult(
        ...
        "Server Status",
        "fail",
        "Function server is not running",
  ...
  generateReport(): void {
    ...
    categories.forEach((category) => {
      ...
      console.log(
        `   Summary: ${categoryPass} pass, ${categoryFail} fail, ${categoryWarning} warning`,
      );
      console.log();
    ...
    if (totalFail === 0) {
      console.log(
        "🎉 Environment validation PASSED! You're ready to run integration tests.",
      );
    } else if (totalFail <= 2 && totalWarning <= 3) {
      console.log(
        "⚠️  Environment validation passed with issues. Review failures before testing.",
      );
    } else {
      console.log(
        "❌ Environment validation FAILED. Please fix the issues before proceeding.",
      );
      console.log();
