...
import {
  generateRealTestProperties,
  REAL_PROPERTY_SCENARIOS,
} from "./real-property-scenarios.ts";
...
REAL_PROPERTY_SCENARIOS.forEach((scenario, i) => {
  ...
  console.log(
    `   Price Range: $${scenario.priceRange[0].toLocaleString()} - $${
      scenario.priceRange[1].toLocaleString()
    }`,
  );
  console.log();
});
...
sampleProperties.forEach((prop, i) => {
  ...
  console.log(
    `   Expected: ${prop.expectedBedrooms} bed, ${prop.expectedBathrooms} bath, $${prop.expectedPrice.toLocaleString()}/month`,
  );
