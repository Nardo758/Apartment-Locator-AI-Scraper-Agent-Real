...
serve(async (req: Request) => {
  try {
    ...
    const messages = [
      ...
      },
      {
        role: "user",
        content:
          `Extract apartment data from this ${source} page HTML:\n\n${htmlContent}`,
      },
    ];
    ...
    if (!openaiKey) {
      return new Response(
        JSON.stringify({ status: "error", message: "OPENAI_API_KEY not set" }),
        { status: 500, headers: { "content-type": "application/json" } },
      );
    }
    ...
    const usage = ((aiResponse as unknown) as { usage?: OpenAIUsage })?.usage ??
      null;
    const respAny = aiResponse as unknown as {
    ...
    if (!validateAiResult(result)) {
      return new Response(
        JSON.stringify({
          status: "error",
          error: "AI result failed validation",
          data: result,
        }),
        {
          status: 422,
  ...
  const supabase = createTypedClient(SUPABASE_URL, SUPABASE_KEY);
    ...
    try {
      if (usage && typeof usage === "object") {
        const promptTokens = Number(
          (usage as Record<string, unknown>)["prompt_tokens"] ?? 0,
        );
        ...
        const modelKey = String(
          completionParams.model ?? recommendedConfig.model ?? "gpt-3.5-turbo",
        );
        const pricing = MODEL_PRICING[modelKey] ??
          { prompt: 0.0015, completion: 0.002 };
        const estimatedCost = ((promptTokens * pricing.prompt) +
    ...
    const msg = (err as unknown instanceof Error)
