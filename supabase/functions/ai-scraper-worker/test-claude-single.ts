...
async function testSinglePropertyWithClaude(): Promise<void> {
  ...
  console.log(
    `🔧 Using model: ${
      Deno.env.get("CLAUDE_MODEL") || "claude-3-haiku-20240307"
    }`,
  );
  ...
  try {
        ...
        console.error(
          `Raw response: ${
            typeof result.raw === "string"
              ...
              : JSON.stringify(result.raw).substring(0, 200) + "..."
          }`,
        );
      }
      return;
    ...
    if (result.usage) {
      ...
      if (usage.estimated_cost) {
        ...
        console.log(
          `     📈 100 properties estimate: $${(cost * 100).toFixed(2)}`,
        );
      }
    }
    ...
    if (result.data) {
      ...
      console.log();
      console.log(
        `📈 Claude Extraction Accuracy: ${passedValidations}/${validations.length} (${
          accuracy.toFixed(1)
        }%)`,
      );
      ...
      if (accuracy >= 80) {
        ...
        console.log(
          "❌ Poor extraction accuracy. Check the Claude prompt or validation logic.",
        );
      }
    }
