...
async function analyzeFunction(): Promise<void> {
  ...
  try {
    ...
    const functions = lines
      ...
      .map(({ line, index }) => ({
        name: line.match(/function (\w+)|const (\w+)/)?.[1] ||
          line.match(/const (\w+)/)?.[1] || "unknown",
        line: index + 1,
        content: line.trim(),
      }));
    ...
    const validateFunction = lines.find((line) =>
      line.includes("validateAiResult")
    );
    if (validateFunction) {
      console.log(`\n✅ Validation Function:`);
      ...
      const requiredFieldsLine = lines.find((line) =>
        line.includes("requiredFields")
      );
    ...
    console.log(
      `   Validation Error: { status: "error", error: "...", data?: {...} }`,
    );

    console.log(`\n🎯 Testing Recommendations:`);
    console.log(
      `   1. ✅ Run validation tests: deno run --allow-read test-validation.ts`,
    );
    console.log(
      `   2. 🌐 Run integration tests: deno run --allow-net --allow-env test-ai-scraper.ts`,
    );
    console.log(
