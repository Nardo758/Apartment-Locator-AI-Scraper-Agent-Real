/**
 * Metrics Collection and Analysis for Real Estate Scraper Command Center
 *
...
export class Metrics {
    ...
    try {
      ...
      if (logs && logs.length > 0) {
        ...
        if (scrapeTimes.length > 0) {
          avgScrapeTime = scrapeTimes.reduce((sum: number, time: number) =>
            sum + time, 0) / scrapeTimes.length;
        }
        ...
        if (responseTimes.length > 0) {
          avgResponseTime = responseTimes.reduce((sum: number, time: number) =>
            sum + time, 0) / responseTimes.length;
        }

        // Calculate error rate
        const errors = logs.filter((log: ScrapeLog) =>
          log.status === "error"
        ).length;
      ...
      const recentLogs = logs?.filter((log: ScrapeLog) =>
  ...
  private async getBusinessMetrics(): Promise<SystemMetrics["business"]> {
    ...
    try {
      ...
      const activeSources = sources
        ? new Set(sources.map((s: Source) => s.source)).size
        : 0;
  ...
  private async getCostMetrics(): Promise<SystemMetrics["costs"]> {
    ...
    try {
      ...
      const weeklyCostAmount =
        weeklyCosts?.reduce((sum: number, cost: CostRecord) =>
          sum + (cost.estimated_cost || 0), 0) || 0;
      const monthlyCostAmount =
        monthlyCosts?.reduce((sum: number, cost: CostRecord) =>
          sum + (cost.estimated_cost || 0), 0) || 0;

      const propertiesScrapedToday = dailyCost?.properties_scraped || 0;
      const costPerProperty = propertiesScrapedToday > 0
        ? dailyCostAmount / propertiesScrapedToday
        : 0;

      // Calculate cost efficiency (properties per dollar)
      const costEfficiency = dailyCostAmount > 0
        ? propertiesScrapedToday / dailyCostAmount
        : 0;
    ...
    try {
      ...
      if (recentLogs && recentLogs.length > 0) {
        ...
        if (confidenceScores.length > 0) {
          avgConfidence =
            confidenceScores.reduce((sum: number, score: number) =>
              sum + score, 0) / confidenceScores.length;
        }
        ...
        if (responseTimes.length > 0) {
          avgSpeed = responseTimes.reduce((sum: number, time: number) =>
            sum + time, 0) / responseTimes.length;
        }
      ...
      const tokenEfficiency = requestsToday > 0
  ...
  private generateMetricAlerts(metrics: SystemMetrics): MetricAlert[] {
    ...
    // Check error rate
    if (
      metrics.performance.error_rate >= this.alertThresholds.error_rate.critical
    ) {
      alerts.push({
        metric: "error_rate",
        ...
        severity: "critical",
        message:
          `Critical error rate detected: ${metrics.performance.error_rate}%`,
        timestamp,
      });
    } else if (
      metrics.performance.error_rate >= this.alertThresholds.error_rate.warning
    ) {
      alerts.push({
        metric: "error_rate",
        current_value: metrics.performance.error_rate,
    ...
    if (
      metrics.performance.success_rate <=
        this.alertThresholds.success_rate.critical
    ) {
      alerts.push({
        metric: "success_rate",
    ...
    // Check data quality
    if (
      metrics.business.data_quality_score <=
        this.alertThresholds.data_quality.critical
    ) {
      alerts.push({
        metric: "data_quality",
        ...
        severity: "critical",
        message:
          `Poor data quality detected: ${metrics.business.data_quality_score}%`,
        timestamp,
      });
    }
  ...
  private calculateTrendStatistics(
    data: TrendData[],
  ...
  private generatePerformanceRecommendations(
    _benchmarks: Record<string, unknown>,
  ): string[] {
