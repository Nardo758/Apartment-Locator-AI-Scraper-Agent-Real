...
async function searchApartmentsWithSerp(params: SerpSearchParams) {
  ...
  const searchUrl = `https://serpapi.com/search.json?q=${
    encodeURIComponent(params.query)
  }&location=${
    encodeURIComponent(params.location)
  }&num=${num}&api_key=${serpApiKey}`;
...
serve(async (req: Request) => {
  try {
    ...
    if (!body.query || !body.location) {
      return new Response(
        JSON.stringify({ status: "error", error: "Missing query or location" }),
        { status: 400, headers: { "content-type": "application/json" } },
      );
    }

    const results = await searchApartmentsWithSerp({
      query: body.query,
