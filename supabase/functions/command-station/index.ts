...
async function routeRequest(
  ...
  switch (`${method}:${action}`) {
    ...
    case "GET:trends": {
      ...
      const timeRange = urlParams.get("range") as "1h" | "24h" | "7d" | "30d" ||
        "24h";
      const granularity =
        urlParams.get("granularity") as "5m" | "1h" | "6h" | "1d" || "1h";
...
function handleHelp(): Response {
  const help = {
    ...
    endpoints: {
      ...
      analytics: {
        "GET /trends/{metric}?range=24h&granularity=1h":
    ...
    examples: {
      ...
      update_config:
        "curl -X POST https://your-project.supabase.co/functions/v1/command-center/config -d '{\"batchSize\": 25}'",
      run_batch:
...
function handleExportConfig(): Response {
  try {
    ...
    return new Response(configExport, {
      ...
      headers: {
        "Content-Type": "application/json",
        "Content-Disposition": `attachment; filename="scraper-config-${
...
function getAvailableEndpoints(): string[] {
  return [
    ...
    "GET /health",
    "GET /activity",
    "POST /enable-scraping",
    "POST /disable-scraping",
    "POST /run-now",
    "POST /emergency-stop",
    "GET /config",
    "POST /config",
