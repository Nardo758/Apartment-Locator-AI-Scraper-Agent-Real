...
export async function runProcessor() {
  ...
  const jobs = raw.filter((r): r is ScrapingJob =>
  ...
  for (const job of jobs) {
    try {
      ...
      const extractionResult = await processPropertyByType(
        job["url"] as string,
        html,
        websiteType,
      );
      ...
      if (
        ...
        if (Array.isArray(extractionResult)) {
          ...
          extractionResult.units && Array.isArray(extractionResult.units)
        ) {
      ...
      if (success && apartments.length > 0) {
        for (const apartment of apartments) {
          try {
            ...
            const apartmentExternalId = `extracted_${Date.now()}_${
            ...
            const apartmentData = {
              ...
              source: job["source"],
              title: apartment.title || apartment.unit_type ||
                `Apartment from ${job["source"]}`,
