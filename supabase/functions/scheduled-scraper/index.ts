...
serve(async (req: Request) => {
  ...
  const supabase = createTypedClient(supabaseUrl, supabaseKey);
    ...
    if (dailyCost > config.cost_limit_daily && !force) {
      console.log(
        `⚠️ Daily cost limit exceeded: $${dailyCost} > $${config.cost_limit_daily}`,
      );
    ...
    const propertiesToScrape = await getNextScrapingBatch(
      supabase,
      batchSize,
      region,
    );
    ...
    console.log(
      `📊 Found ${propertiesToScrape.length} property sources to scrape`,
    );
    ...
    for (const propertySource of propertiesToScrape) {
      try {
        console.log(
          `🔍 Processing: ${propertySource.property_name} (${propertySource.url})`,
        );
        ...
        console.log(
          `✅ Processed ${propertySource.property_name}: ${result.properties_found} properties, $${result.cost}`,
        );
        ...
        if (currentDailyCost + totalCost > config.cost_limit_daily * 0.9) {
          console.log(
            "⚠️ Approaching daily cost limit, stopping batch processing",
          );
          break;
        }
      } catch (error) {
    ...
    const intelligenceAdded = results.filter((r) =>
      r.intelligence_added
    ).length;
...
function getDeploymentConfig(): DeploymentConfig {
  ...
  return {
    ...
    claude_analysis_enabled:
      Deno.env.get("CLAUDE_ANALYSIS_ENABLED") !== "false",
    ...
    max_consecutive_failures: parseInt(
      Deno.env.get("MAX_CONSECUTIVE_FAILURES") || "5",
    ),
    rate_limit_delay_ms: parseInt(
      Deno.env.get("RATE_LIMIT_DELAY_MS") || "1000",
    ),
    regions: (Deno.env.get("SCRAPING_REGIONS") || "atlanta,new-york,chicago")
      .split(","),
...
async function processPropertySource(
  ...
  try {
    ...
    const scraperResponse = await fetch(
      `${Deno.env.get("SUPABASE_URL")}/functions/v1/ai-scraper-worker`,
      {
        method: "POST",
        headers: {
    ...
    if (
      claudeEnabled && scraperResult.success &&
      scraperResult.properties?.length > 0
    ) {
      try {
        const intelligenceResponse = await fetch(
          `${Deno.env.get("SUPABASE_URL")}/functions/v1/property-researcher`,
          {
            method: "POST",
            headers: {
              "Authorization": `Bearer ${
