...
class PopupManager {
  ...
  init() {
    ...
    document.getElementById("clear-actions").addEventListener(
      "click",
      () => this.clearActions(),
    );
    document.getElementById("view-sessions").addEventListener(
      "click",
      () => this.viewSessions(),
    );
    this.exportCurrentBtn.addEventListener(
      "click",
  ...
  updateUI() {
    ...
    this.statusDiv.className = this.isRecording
      ? "status recording"
      : "status stopped";
    this.statusDiv.textContent = this.isRecording
      ? "Recording..."
      : "Not Recording";
    ...
    this.recordingInfo.classList.toggle(
      "hidden",
      !this.isRecording && this.actionCount === 0,
    );
    this.updateActionCount();

    // Update export button
    this.exportCurrentBtn.classList.toggle(
      "hidden",
      this.isRecording || this.actionCount === 0,
    );
  }
  ...
  displaySessions(sessions) {
    ...
    if (Object.keys(sessions).length === 0) {
      sessionsList.innerHTML =
        '<div class="session-item">No saved sessions</div>';
      return;
    }
    ...
    for (const [sessionId, session] of Object.entries(sessions)) {
      ...
      meta.textContent = `${actionCount} actions • ${duration}s • ${
