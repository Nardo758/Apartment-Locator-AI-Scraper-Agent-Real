...
async function run() {
  try {
    ...
    let { _data, error } = await sb.from("apartments").select("external_id")
      .limit(5);
    if (error) {
      console.log(
        "apartments select error, falling back to todos:",
        error.message,
      );
      ({ _data, error } = await sb.from("todos").select("id").limit(5));
    }
    ...
    if (process.argv.includes("--upsert")) {
      console.log(
        "Performing harmless upsert into `scraping_debug` (will create table if missing)",
      );
      const payload = {
      ...
      const { error: upsertError } = await sb.from("scraping_debug").upsert(
        payload,
