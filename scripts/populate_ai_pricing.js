...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  console.error(
    "Please set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY environment variables (or .env.local).",
  );
  process.exit(1);
}
...
function computePricingHeuristic(property) {
  ...
  const sqft = typeof property.square_footage === "number"
    ? property.square_footage
    : (typeof property.square_footage === "string"
  ...
  const amenities = Array.isArray(property.amenities)
    ...
    : 0;
  const unitFeatures = Array.isArray(property.unit_features)
    ? property.unit_features.length
    : 0;
  ...
  const concession = typeof property.concession_value === "number"
    ? property.concession_value
    : 0;
...
async function runBackfill() {
  console.log(
    `populate_ai_pricing: dryRun=${dryRun} batchSize=${batchSize} computePercentile=${computePercentile}`,
  );
  ...
  while (true) {
    const { _data, error } = await supabase
      ...
      .select(
        "id, external_id, current_price, price, square_footage, amenities, unit_features, market_velocity, concession_value, zip_code",
      )
      .is("ai_price", null)
      .limit(batchSize);
    ...
    for (const p of data) {
      ...
      if (computePercentile && p.external_id) {
        try {
          ...
          if (prErr) {
            ...
            pr && Array.isArray(pr) && pr.length > 0 && pr[0] &&
            typeof pr[0] === "number"
          ) {
            percentile = pr[0];
      ...
      if (dryRun) {
        console.log(
          "DRY RUN - would update id=",
          p.id,
          "payload=",
          updatePayload,
        );
      } else {
        const { error: upErr } = await supabase.from("scraped_properties")
          .update(updatePayload).eq("id", p.id);
