...
(async () => {
  ...
  const qUrl = SUPABASE_URL.replace(/\/$/, '') + `/rest/v1/scraped_properties?external_id=eq.${encodeURIComponent(external_id)}&select=*`;
  ...
  const phUrl = SUPABASE_URL.replace(/\/$/, '') + `/rest/v1/price_history?external_id=eq.${encodeURIComponent(external_id)}&select=*`;
