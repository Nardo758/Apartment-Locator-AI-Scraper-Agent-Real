...
const argv = yargs(hideBin(process.argv))
  ...
  .option("redact", {
    type: "boolean",
    default: false,
    description: "Redact emails and phone numbers from payload before export",
  })
  .option("redact-fields", {
    type: "string",
    description:
      "Comma-separated list of fields to redact (e.g. contact_email,phone,owner_name,address)",
  })
  .option("address-hash", {
    type: "string",
    default: "sha1",
    description:
      "How to handle address when redacting: sha1 or geohash (sha1 default)",
  })
  ...
  .option("mock", {
    type: "boolean",
    default: false,
    description: "Run in mock mode using fixture rows (no DB required)",
  })
  .argv;
...
if (!mockMode) {
  if (!SUPABASE_URL || !SUPABASE_KEY) {
    console.error(
      "Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY env vars to run this script (or use --mock)",
    );
    process.exit(1);
  }
...
function normalizeErrorType(err) {
  ...
  try {
    ...
    if (err.message) {
      return String(err.message).replace(/[^a-zA-Z0-9_\-]/g, "_").slice(0, 80);
    }
...
function redactPII(obj, options = {}) {
  ...
  function redactFieldsInPayload(payload) {
    ...
    const out = { ...payload };
    // emails
    if (redactFields.has("contact_email") && out.contact_email) {
      out.contact_email = "[REDACTED_EMAIL]";
    }
    if (redactFields.has("manager_email") && out.manager_email) {
      out.manager_email = "[REDACTED_EMAIL]";
    }
    if (redactFields.has("agent_email") && out.agent_email) {
      out.agent_email = "[REDACTED_EMAIL]";
    }

    // phones
    if (redactFields.has("phone") && out.phone) out.phone = "[REDACTED_PHONE]";
    if (redactFields.has("contact_phone") && out.contact_phone) {
      out.contact_phone = "[REDACTED_PHONE]";
    }
    if (redactFields.has("manager_phone") && out.manager_phone) {
      out.manager_phone = "[REDACTED_PHONE]";
    }

    // names
    if (redactFields.has("owner_name") && out.owner_name) {
      out.owner_name = "[REDACTED_NAME]";
    }
    if (redactFields.has("contact_name") && out.contact_name) {
      out.contact_name = "[REDACTED_NAME]";
    }
    if (redactFields.has("agent_name") && out.agent_name) {
      out.agent_name = "[REDACTED_NAME]";
    }
...
async function main() {
  ...
  if (mockMode) {
    ...
    const { _data, error } = await supabase
      .from("failed_scrapes")
      .select(
        "id, external_id, payload, error, created_at, training_priority, training_notes",
      )
  ...
  console.log(
    `Found ${rows.length} failed_scrapes rows, grouped into ${
  ...
  for (const [errorType, items] of Object.entries(groups)) {
    for (let i = 0; i < items.length; i += batchSize) {
      ...
      const redactFieldsArg = argv["redact-fields"]
        ? new Set(String(argv["redact-fields"]).split(",").map((s) => s.trim()))
        : new Set();
      const redactOptions = {
        redactFields: redactFieldsArg,
        addressHash: argv["address-hash"],
      };
      const redaction = redact ? redactPII(null, redactOptions) : null;

      const exportedRecords = batch.map((r) => {
        const payload = redact
          ? redaction.redactFieldsInPayload(r.payload)
          : r.payload;
        ...
        const record = {
          ...
          listing_url: payload && payload.listing_url
            ? payload.listing_url
            : null,
      ...
      const jsonlContent = exportedRecords.map((r) =>
        JSON.stringify(r)
      ).join("\n") + "\n";
      ...
      console.log(
        `Wrote ${jsonlPath} (${exportedRecords.length} items), checksum ${checksum}`,
      );

      if (!dryRun) {
        const ids = batch.map((b) => b.id);
        const { error: updateError } = await supabase.from("failed_scrapes")
          .update({
            training_batch_id: batchId,
        ...
        if (updateError) {
          console.error(
            "Error updating training fields on failed_scrapes:",
            updateError,
          );
        } else {console.log(
            `Marked ${ids.length} rows as queued with batch id ${batchId}`,
          );}
      } else {
        console.log(
          `[dry-run] Would mark ${batch.length} rows as queued with batch id ${batchId}`,
        );
      }
    }
  }
