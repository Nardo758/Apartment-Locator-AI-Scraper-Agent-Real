...
(async function () {
  try {
    ...
    const { data: upserted, error: upErr } = await supabase.from(
      "scraped_properties",
    ).upsert(property, { onConflict: ["property_id"] }).select();
    if (upErr) {
      console.error("Error upserting property", upErr);
      process.exit(1);
    }
    ...
    const { data: qData, error: qErr } = await supabase.from("scraping_queue")
      .insert(queueRow).select();
    if (qErr) {
      console.error("Error inserting queue row", qErr);
      process.exit(1);
    }
