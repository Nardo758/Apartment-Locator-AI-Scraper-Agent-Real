...
async function verifyPreMigration() {
  console.log(
    "🔍 Verifying database schema before location fields migration...\n",
  );
  ...
  try {
    ...
    if (error) {
      ...
      results.push({
        test: "Table structure query",
        status: "✅ PASS",
        details: `${data.length} columns found`,
      });
      ...
      for (const col of locationColumns) {
        if (existingColumns.includes(col)) {
          results.push({
            ...
            status: "⚠️  WARN",
            details:
              "Column already exists - migration will be skipped for this column",
          });
        } else {
          results.push({
    ...
    if (error) {
      ...
      if (locationIndexes.length > 0) {
        results.push({
          ...
          status: "⚠️  WARN",
          details:
            `${locationIndexes.length} location-related indexes already exist`,
        });
      } else {
        results.push({
          test: "Location indexes check",
...
async function generatePreMigrationSnapshot() {
  ...
  try {
    ...
    // Save snapshot
    const filename = `verification_pre_migration_${
