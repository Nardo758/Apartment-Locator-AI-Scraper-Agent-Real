...
async function upsertTest() {
  ...
  const { error: apartmentError } = await sb.from("apartments").upsert(
    apartmentData,
  ...
  if (process.argv.includes("--frontendsync")) {
    ...
    const { error: propErr } = await sb.from("properties").upsert(frontend, {
      onConflict: "external_id",
    });
  ...
  const { _data, error } = await sb.from("apartments").select("*").eq(
    "external_id",
    externalId,
  ).limit(1);
...
upsertTest().then(() => process.exit(0)).catch((e) => {
  console.error(e);
  process.exit(1);
});
