...
async function verifyMigrationSuccess() {
  ...
  try {
    ...
    if (error) {
      ...
      for (const expectedConstraint of expectedConstraints) {
        const found = data.find((constraint) =>
          constraint.constraint_name === expectedConstraint
        );
  ...
  try {
    ...
    if (testResult.error) {
      ...
      if (
        checks.latitude_check === "latitude_constraint_exists" &&
        checks.longitude_check === "longitude_constraint_exists"
      ) {
        results.push({
          test: "Constraint validation",
          status: "✅ PASS",
          details: "Constraints are active",
        });
      } else {
        results.push({
          test: "Constraint validation",
          status: "❌ FAIL",
          details: "Some constraints missing",
        });
        allPassed = false;
      }
    }
  ...
  for (const result of results) {
    console.log(
      `${result.status} ${result.test.padEnd(35)} | ${result.details}`,
    );
  }
  ...
  if (allPassed) {
    console.log(
      "🎉 Migration verification passed! All changes applied successfully.",
    );
    return true;
  } else {
    console.log(
      "❌ Migration verification failed. Some issues need to be addressed.",
    );
    return false;
  }
}
...
async function generatePostMigrationSnapshot() {
  ...
  try {
    ...
    // Get indexes
    const { data: indexData, error: _indexError } = await supabase.rpc(
      "exec_sql",
      {
        sql: `
        SELECT indexname, indexdef
    ...
    // Get constraints
    const { data: constraintData, error: _constraintError } = await supabase
      .rpc("exec_sql", {
        sql: `
        SELECT constraint_name, check_clause
        FROM information_schema.check_constraints 
    ...
    const { data: sampleData, error: _sampleError } = await supabase
      .from("scraped_properties")
      .select(
        "id, external_id, address, city, state, latitude, longitude, zip_code",
      )
      .limit(5);
    ...
    const snapshot = {
      ...
      checks: {
        ...
        locationIndexExists: (indexData || []).some((idx) =>
          idx.indexname === "idx_scraped_properties_location"
        ),
        zipIndexExists: (indexData || []).some((idx) =>
          idx.indexname === "idx_scraped_properties_zip_code"
        ),
      },
    };

    // Save snapshot
    const filename = `verification_post_migration_${
...
function compareSnapshots(preFile, postFile) {
  ...
  console.log(
    `Row Count: ${pre.rowCount} → ${post.rowCount} (${
      post.rowCount === pre.rowCount ? "unchanged ✅" : "changed ⚠️"
    })`,
  );
  console.log(
    `Total Columns: ${pre.schema.length} → ${post.schema.length} (${
      post.schema.length > pre.schema.length
        ? "added +" + (post.schema.length - pre.schema.length) + " ✅"
        : "unchanged"
    })`,
  );
  ...
  console.log(
    `\n📍 Location Features: ${
      post.checks.hasLatitude && post.checks.hasLongitude &&
        post.checks.hasZipCode
        ? "✅ Ready"
        : "❌ Incomplete"
    }`,
  );
  console.log(
    `🗂️  New Indexes: ${
      post.checks.locationIndexExists && post.checks.zipIndexExists
        ? "✅ Created"
        : "❌ Missing"
    }`,
  );
...
async function main() {
  ...
  console.log(
    `🚀 Location Fields Migration - Post-Migration Verification (${env})\n`,
  );
  ...
  const preFiles = fs.readdirSync(".").filter((f) =>
    f.startsWith("verification_pre_migration_")
  );
  if (preFiles.length > 0) {
    const latestPre = preFiles.sort().pop();
    const postFiles = fs.readdirSync(".").filter((f) =>
      f.startsWith("verification_post_migration_")
    );
