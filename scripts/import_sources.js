...
async function initSupabase() {
  try {
    ...
    const createClient = mod.createClient ||
      (mod.default && mod.default.createClient);
    ...
    const SUPABASE_KEY = cli.key || process.env.SUPABASE_SERVICE_ROLE_KEY ||
    ...
    const urlLooksValid = typeof SUPABASE_URL === "string" &&
      SUPABASE_URL.startsWith("http") && SUPABASE_URL.includes("supabase");

    if (!urlLooksValid) {
      console.warn(
        "Supabase URL appears malformed or missing. Skipping DB init.",
      );
      console.log(
        "Provided SUPABASE_URL value present?",
        !!(cli.url || process.env.SUPABASE_URL),
      );
    ...
    console.warn(
      "Warning: failed to import @supabase/supabase-js (ESM import). DB upsert will be skipped.",
    );
...
async function main() {
  ...
  if (!supabase) {
    ...
    const hasKey = !!(process.env.SUPABASE_SERVICE_ROLE_KEY ||
      process.env.SUPABASE_SERVICE_ROLE);
    console.log(
      "Supabase client not initialized. SUPABASE_URL set?",
      hasUrl,
      "SUPABASE_KEY set?",
      hasKey,
    );
    console.log(
      "If both are true, ensure @supabase/supabase-js is installed (npm install @supabase/supabase-js) and try again.",
    );
  }
  ...
  fs.createReadStream(csvPath)
    ...
    .on("end", async () => {
      ...
      if (supabase) {
        console.log(
          "Supabase client detected — upserting into table `sources`",
        );
        // Upsert in batches
        const chunkSize = 100;
        for (let i = 0; i < records.length; i += chunkSize) {
          ...
          if (dryRun) {
            console.log(
              `[DRY RUN] Would upsert ${chunk.length} rows:`,
              JSON.stringify(chunk.slice(0, 3), null, 2),
              "...",
            );
          } else {
            const { data: _data, error } = await supabase.from("sources")
        ...
        console.log(
          "Install @supabase/supabase-js and set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY env vars to enable upsert.",
        );
      }
    })
    .on("error", (err) => {
