...
async function displayStatus() {
  ...
  console.log(
    `Properties with ZIP codes: ${status.properties_with_zip.toLocaleString()} (${status.zip_completion_rate}%)`,
  );
  console.log(
    `Properties with coordinates: ${status.complete_coordinates.toLocaleString()} (${status.coordinate_completion_rate}%)`,
  );
  console.log("");
  ...
  if (needsZip > 0) {
    console.log(
      `🎯 Next Steps: ${needsZip.toLocaleString()} properties need ZIP codes`,
    );
  ...
  if (needsCoords > 0) {
    console.log(
      `🗺️  Next Steps: ${needsCoords.toLocaleString()} properties need coordinates`,
    );
...
async function runCompleteBackfill() {
  ...
  if (initialStatus.zip_completion_rate < 100) {
    ...
    console.log(
      `✅ ZIP backfill complete: ${zipResults.updated} properties updated`,
    );
  } else {
  ...
  if (initialStatus.coordinate_completion_rate < 100) {
    ...
    while (geocodeRounds < maxRounds) {
      ...
      const remaining = currentStatus.total_properties -
        currentStatus.complete_coordinates;
  ...
  if (finalStatus && initialStatus) {
    const zipImprovement = finalStatus.zip_completion_rate -
      initialStatus.zip_completion_rate;
    const coordImprovement = finalStatus.coordinate_completion_rate -
      initialStatus.coordinate_completion_rate;
...
async function runContinuousBackfill() {
  ...
  console.log(
    `Running every ${intervalMinutes} minutes. Press Ctrl+C to stop.\n`,
  );
...
async function cleanupLocationData() {
  ...
  try {
    ...
    const { data: invalidCoords, error: coordError } = await supabase.rpc(
      "exec_sql",
      {
        sql: `
    ...
    if (coordError) {
      ...
      console.log(
        `🗂️  Cleaned ${invalidCoords.length} properties with invalid coordinates`,
      );
      cleanedCount += invalidCoords.length;
    }

    // Clean up invalid ZIP codes
    const { data: invalidZips, error: zipError } = await supabase.rpc(
      "exec_sql",
      {
        sql: `
        UPDATE scraped_properties 
    ...
    if (zipError) {
      ...
      console.log(
        `📮 Cleaned ${invalidZips.length} properties with invalid ZIP codes`,
      );
...
async function main() {
  ...
  console.log(
    `🎯 Location Data Backfill Orchestrator - ${command.toUpperCase()}\n`,
  );
