...
async function geocodePropertiesOnce() {
  console.log(
    `Geocoder provider=${provider} batchSize=${batchSize} dryRun=${dryRun}`,
  );
  ...
  try {
    ...
    console.warn(
      "Initial select with zip_code failed, retrying without zip_code:",
      err?.message || err,
    );
  }
  ...
  for (const p of properties) {
    try {
      const addrParts = [p.address, p.city, p.state, p.zip_code].filter(
        Boolean,
      );
      const fullAddress = addrParts.join(", ");
      if (!fullAddress) {
        console.log(
          `Skipping property id=${p.id} external_id=${p.external_id} because address missing`,
        );
        continue;
      }
      ...
      if (!results || results.length === 0) {
        console.log(
          `No geocode result for id=${p.id} address='${fullAddress}'`,
        );
        continue;
      }
      ...
      if (!dryRun) {
        const { error: upErr } = await supabase
          ...
          .eq("id", p.id);
        if (upErr) {
          console.error(`Failed to update property id=${p.id}:`, upErr);
        }
      }
    } catch (_e) {
