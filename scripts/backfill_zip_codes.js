  ...
  while (hasMore) {
    try {
      ...
      for (const property of properties) {
        ...
        if (extractedZip) {
          console.log(
            `📍 Property ${property.external_id}: Found ZIP ${extractedZip} in "${property.address}"`,
          );
          ...
          if (!dryRun) {
            ...
            if (updateError) {
              console.error(
                `❌ Failed to update property ${property.id}:`,
                updateError,
              );
            } else {
              batchUpdated++;
          ...
          console.log(
            `⚪ Property ${property.external_id}: No ZIP found in "${property.address}"`,
          );
        }
      }
...
async function generateZipReport() {
  ...
  try {
    ...
    console.log(
      `Total properties needing ZIP codes: ${sampleProperties.length}`,
    );
    console.log(
      `Estimated successful extractions: ~${
        Math.round((sampleProperties.length * estimatedSuccess) / 100)
      }`,
    );
  } catch (_e) {
...
async function validateZipCodes() {
  ...
  try {
    ...
    console.log(
      `Validation rate: ${Math.round((valid / (valid + invalid)) * 100)}%`,
    );
    ...
    if (issues.length > 0) {
      ...
      issues.slice(0, 10).forEach((issue) => {
        console.log(
          `  ${issue.external_id}: "${issue.zip_code}" - ${issue.issue}`,
        );
      });
