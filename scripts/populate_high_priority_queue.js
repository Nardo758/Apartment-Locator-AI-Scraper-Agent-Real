...
async function populateHighPriorityQueue() {
  ...
  sources.forEach((source) =>
    console.log(`  - ${source.name} (${source.priority})`)
  );
  console.log();
  ...
  if (insertError) {
    ...
    console.log(
      `✅ Successfully inserted ${data.length} high-priority scraping jobs`,
    );
    console.log("\nJobs created:");
    data.forEach((job) => {
      console.log(
        `  - ${job.external_id}: ${job.source} (${job.url}) - Priority: ${job.priority_tier}`,
      );
    });
  }
}
