...
This document outlines the plan to add latitude, longitude, and zip_code fields
to the `scraped_properties` table to enable location-based features and mapping
functionality.
...
- `idx_scraped_properties_location`: Composite index on (latitude, longitude)
...
1. **Backup Database**
   ...
   backup_pre_location_migration_$(date +%Y%m%d_%H%M%S).sql

2. **Verify Current Schema**
   # Run verification script
   node scripts/verify_schema_pre_migration.js

3. **Check Disk Space** -- Estimate space needed for new columns (run this
   query) SELECT COUNT(_) as total_properties, pg_size_pretty(COUNT(_) * (8 +
   8 + 50)) as estimated_additional_space FROM scraped_properties;
...
1. **Verify Migration Success** node scripts/verify_migration_success.js
