...
async function checkScrapedPropertiesSchema() {
  ...
  try {
    ...
    columnsResult.rows.forEach((col) => {
      console.log(
        `${col.column_name}: ${col.data_type} | nullable: ${col.is_nullable} | default: ${col.column_default} | generated: ${col.is_generated}`,
      );
    });
    ...
    constraintsResult.rows.forEach((con) => {
      console.log(
        `${con.constraint_name}: ${con.constraint_type} on ${con.column_name}`,
      );
    });
  } catch (error) {
