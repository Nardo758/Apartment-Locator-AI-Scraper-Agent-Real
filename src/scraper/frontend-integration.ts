...
export class ScraperFrontendIntegration {
  ...
  async processScraperResults(scraperResult: ScraperResult): Promise<{
    ...
    if (!scraperResult.success || !scraperResult.properties?.length) {
      return {
        processed: 0,
        errors: ["No properties to process"],
        frontend_properties_created: 0,
      };
    }

    console.log(
      `🔄 Processing ${scraperResult.properties.length} scraped properties for frontend`,
    );

    try {
      // 1. First, ensure scraped properties are in the scraped_properties table
      const scrapedProperties = await this.upsertScrapedProperties(
        scraperResult.properties,
      );
      processed = scrapedProperties.length;

      // 2. Transform and upsert to frontend properties table
      frontendPropertiesCreated = await frontendDataService
        .bulkTransformAndUpsert(scrapedProperties);
  ...
  private async updateLocationIntelligence(
    ...
    try {
      ...
      const rentPerSqft = sqfts.length > 0
        ? prices.reduce((sum, price, i) => sum + (price / (sqfts[i] || 1)), 0) /
          sqfts.length
        : null;

      // Calculate market velocity based on pricing trends
      const marketVelocity = await this.calculateMarketVelocity(
        location,
        properties,
      );
  ...
  private generateMarketRecommendations(
    ...
    if (concessionPrevalence > 20) {
      recommendations.negotiation = {
        potential: "high",
        message:
          "High concession rate suggests strong negotiation opportunities",
      };
    } else if (concessionPrevalence < 5) {
      recommendations.urgency = {
        level: "high",
        message:
          "Low concession rate suggests competitive market - act quickly",
      };
    }
