...
async function testDataTransformation(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `📦 Batch transformation: ${batchResults.length} properties processed`,
    );
...
async function testGeographicSearch(supabase: any): Promise<void> {
  ...
  try {
    ...
    if (error) {
      console.log(
        "⚠️  Geographic search function not available:",
        String(error),
      );
    } else {
      console.log(
        ...
        } properties within ${radiusMiles} miles of Austin`,
      );
      ...
      if (nearbyProperties && nearbyProperties.length > 0) {
        nearbyProperties.slice(0, 3).forEach((prop: any, index: number) => {
          console.log(
            `  ${
              index + 1
            }. ${prop.name} - $${prop.original_price} (${prop.distance_miles} miles)`,
          );
        });
      }
    }
...
async function testAiPriceCalculation(supabase: any): Promise<void> {
  ...
  try {
    ...
    for (const testCase of testCases) {
      ...
      if (error) {
        console.log(
          "⚠️  AI price calculation function not available:",
          String(error),
        );
        break;
      }
      ...
      console.log(
        `💰 ${testCase.bedrooms}BR/${testCase.bathrooms}BA: $${testCase.base_price} → $${aiPrice} (AI estimate)`,
      );
    }
...
async function testDataQuality(supabase: any): Promise<void> {
  ...
  try {
    ...
    if (error || !qualityData || qualityData.length === 0) {
      console.log(
        "⚠️  Data quality assessment skipped - scraped_properties table not accessible",
      );
      ...
      console.log(
        `  Complete Name: ${
          ((mockQuality.has_name / mockQuality.total_properties) * 100).toFixed(
            1,
          )
        }%`,
      );
      console.log(
        `  Complete Address: ${
          ((mockQuality.has_address / mockQuality.total_properties) * 100)
            .toFixed(1)
        }%`,
      );
      console.log(
        `  Valid Price: ${
          ((mockQuality.has_valid_price / mockQuality.total_properties) * 100)
            .toFixed(1)
        }%`,
      );
      console.log(
        `  Has Bedrooms: ${
          ((mockQuality.has_bedrooms / mockQuality.total_properties) * 100)
            .toFixed(1)
        }%`,
      );
      console.log(
        `  Has Bathrooms: ${
          ((mockQuality.has_bathrooms / mockQuality.total_properties) * 100)
            .toFixed(1)
        }%`,
      );
    } else {
      ...
      if (quality.total_properties > 0) {
        console.log(
          `  Complete Name: ${
            ((quality.has_name / quality.total_properties) * 100).toFixed(1)
          }%`,
        );
        console.log(
          `  Complete Address: ${
            ((quality.has_address / quality.total_properties) * 100).toFixed(1)
          }%`,
        );
        console.log(
          `  Valid Price: ${
            ((quality.has_valid_price / quality.total_properties) * 100)
              .toFixed(1)
          }%`,
        );
        console.log(
          `  Has Bedrooms: ${
            ((quality.has_bedrooms / quality.total_properties) * 100).toFixed(1)
          }%`,
        );
        console.log(
          `  Has Bathrooms: ${
            ((quality.has_bathrooms / quality.total_properties) * 100).toFixed(
              1,
            )
          }%`,
        );
...
if (isDeno) {
  runIntegrationTest().then(() => (globalThis as any).Deno.exit?.(0)).catch(
