    ...
    from: (_: string) => ({
      select: (_s: string) => ({
        eq: (_k: string, _v: string) => ({
          maybeSingle: () => ({
            data: firstSeen ? { first_seen_at: firstSeen } : null,
          }),
        }),
      }),
    }),
...
test("scrapePropertyComplete returns enriched object with amenities and property_type", async () => {
  ...
  const input = {
    ...
    address: "123 Main St, 94105",
    description: "Includes hardwood floors and fitness center",
    name: "Lovely apartment",
  };
  const result = await scrapePropertyComplete(
    mockSupabase as any,
    input as any,
  );
