...
describe("amenities parser", () => {
  test("extracts building amenities and unit features", () => {
    const desc =
      `Beautiful unit with hardwood floors, stainless steel appliances and granite countertops. Building has a fitness center and pool, plus a rooftop lounge.`;
    ...
    expect(parsed.amenities).toEqual(
      expect.arrayContaining(["fitness center", "pool", "rooftop"]),
    );
  ...
  test("handles pet policy and parking phrases", () => {
    const desc =
      `Pets allowed: cats allowed and dogs allowed. Assigned parking available.`;
    const parsed = extractAmenities(desc);
