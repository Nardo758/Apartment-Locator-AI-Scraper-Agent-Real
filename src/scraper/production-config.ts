...
export const PRODUCTION_CONFIG: ProductionConfig = {
  ...
  maxConcurrentTransformations: parseInt(
    process.env.MAX_CONCURRENT_TRANSFORMATIONS || "10",
  ),
  transformationTimeoutMs: parseInt(
    process.env.TRANSFORMATION_TIMEOUT_MS || "30000",
  ),
  ...
  // Geographic search
  defaultSearchRadiusMiles: parseInt(
    process.env.DEFAULT_SEARCH_RADIUS_MILES || "10",
  ),
  maxSearchResults: parseInt(process.env.MAX_SEARCH_RESULTS || "100"),

  // Calibration settings (from testing)
  luxuryAmenityPremium: parseFloat(
    process.env.LUXURY_AMENITY_PREMIUM || "0.015",
  ),
  ...
  qualityThresholdMedium: parseFloat(
    process.env.QUALITY_THRESHOLD_MEDIUM || "0.7",
  ),
...
export function validateProductionConfig(config: ProductionConfig): string[] {
  ...
  if (config.qualityThresholdHigh <= config.qualityThresholdMedium) {
    errors.push(
      "qualityThresholdHigh must be greater than qualityThresholdMedium",
    );
  }
