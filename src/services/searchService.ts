...
import type { Apartment } from '../types.ts.ts'
...
interface QueryBuilder {
  ...
  range: (
    ...
    { data: Apartment[] | null; error: unknown; count: number | null }
  >;
}
...
export class SearchService {
  ...
  async searchApartments(filters: SearchFilters = {}): Promise<SearchResult> {
    ...
    if (typeof filters.minPrice === "number") {
      qb = qb.gte("rent_price", filters.minPrice);
    }
    if (typeof filters.maxPrice === "number") {
      qb = qb.lte("rent_price", filters.maxPrice);
    }
    if (typeof filters.bedrooms === "number") {
      qb = qb.eq("bedrooms", filters.bedrooms);
    }
    ...
    if (filters.sort === "price_asc") {
      qb = qb.order("rent_price", { ascending: true });
    }
    if (filters.sort === "price_desc") {
      qb = qb.order("rent_price", { ascending: false });
    }
