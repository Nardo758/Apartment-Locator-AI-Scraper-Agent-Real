...
function heuristicCompute(
  ...
  if (!base && sqft && sqft > 0) {
    base = Math.round(property["price"] ? Number(property["price"]) : 0);
  }
 ...
 */
export async function computeAiPricing(
  ...
  if (customPath) {
    try {
      ...
      console.warn(
        "Failed to load advanced pricing module, falling back to heuristic:",
        String(err),
      );
    }
  }
