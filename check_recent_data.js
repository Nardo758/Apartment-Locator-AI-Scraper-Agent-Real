...
async function checkRecentData() {
  ...
  const { data: recentScraped, error: scrapedError } = await supabase
    ...
    .select(
      "name, address, city, state, current_price, created_at, external_id",
    )
  ...
  if (scrapedError) {
    ...
    recentScraped.forEach((prop, i) => {
      console.log(
        `   ${
          i + 1
        }. ${prop.name} - ${prop.city}, ${prop.state} - $${prop.current_price} (${
  ...
  const { data: recentProps, error: propsError } = await supabase
    .from("properties")
    .select(
      "name, address, city, state, original_price, created_at, external_id",
    )
    .order("created_at", { ascending: false })
    .limit(10);
  ...
  if (propsError) {
    ...
    recentProps.forEach((prop, i) => {
      console.log(
        `   ${
          i + 1
        }. ${prop.name} - ${prop.city}, ${prop.state} - $${prop.original_price} (${
          new Date(prop.created_at).toLocaleString()
        })`,
      );
      console.log(`      External ID: ${prop.external_id}`);
